...
class RecordActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnAddCaffeine.setOnClickListener {
            ...
            if (caffeineInput.isNotEmpty()) {
                ...
                userManager.addCaffeineIntake(caffeineAmount, selectedDate)
                // 입력 필드 초기화
    ...
    private fun updateCaffeineDisplay(date: String, txtCaffeineRecord: TextView) {
        val txtCaffeineDate = findViewById<TextView>(R.id.txtCaffeineDate)
        // 날짜를 '2025-06-02 (월)' 형식으로 변환
        ...
        txtCaffeineDate.text = String.format("%s (%s)", date, dayOfWeek)

        val today = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date())
        val caffeine = if (date == today) {
            userManager.getTodayCaffeineIntake()
        } else {
            val recordDate = date.replace("-", "")
            userManager.getCaffeineRecord(recordDate)
        }
        txtCaffeineRecord.text = "${caffeine}mg 섭취"
    }
}
